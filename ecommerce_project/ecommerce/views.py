...
def my_stores(request):
    ...
    return render(request, "ecommerce/vendor/stores.html", {"stores": stores})
...
def store_detail(request, pk):
    ...
    return render(request, "vendor/store_detail.html", {"store": store})


def my_products(request):
    products = Product.objects.filter(vendor=request.user)
    return render(request, 'ecommerce/vendor/my_products.html', {'products': products})
...
def store_list(request):
    stores = Store.objects.all()
    return render(request, 'ecommerce/stores/list.html', {'stores': stores})

@login_required
def buyer_orders(request):
    user = request.user
    if not hasattr(user, 'is_buyer') or not user.is_buyer:
